...
namespace EveOPreview.UI.Hotkeys
{
    public class GlobalHotKey : IDisposable
    {
        ...
        public static bool RegisterHotKey(string aKeyGestureString, Action<string> aAction)
        {
            var c = new KeyGestureConverter();
            KeyGesture aKeyGesture = (KeyGesture)c.ConvertFrom(aKeyGestureString);
            return RegisterHotKey(aKeyGesture.Modifiers, aKeyGesture.Key, aAction, aKeyGestureString);
        }

        private static bool RegisterHotKey(ModifierKeys aModifier, Key aKey, Action<string> aAction, string aKeyGestureString)
        {
            ...
            if (aRegistered)
            {
                registeredHotKeys.Add(new HotKeyWithAction(aModifier, aKey, aAction, aKeyGestureString));
            }
            return aRegistered;
        }
        ...
        private class HotKeyWithAction
        {
            ...
            public HotKeyWithAction(ModifierKeys modifier, Key key, Action<string> action, string aKeyGestureString)
            {
                Modifier = modifier;
                Key = key;
                Action = action;
                KeyGestureString = aKeyGestureString;
            }

            public ModifierKeys Modifier { get; }
            public Key Key { get; }
            public Action<string> Action { get; }
            public string KeyGestureString { get; }
        }
